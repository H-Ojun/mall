...
public class MypageViewHandler {
    ...
    @StreamListener(KafkaProcessor.INPUT)
    public void whenDeliveryComplete_then_UPDATE_2(@Payload DeliveryComplete deliveryComplete) {
        try {
            if (!deliveryComplete.validate()) return;
                // view 객체 조회

                List<Mypage> mypageList = mypageRepository.findByOrderId(deliveryComplete.getOrderId());
                for(Mypage mypage : mypageList){
                    // view 객체에 이벤트의 eventDirectValue 를 set 함
                    mypage.setStatus(deliveryComplete.getStatus());
                // view 레파지 토리에 save
                mypageRepository.save(mypage);
                }
    ...
    @StreamListener(KafkaProcessor.INPUT)
    public void whenDeliveryComplete_then_UPDATE_4(@Payload DeliveryComplete deliveryComplete) {
        try {
            if (!deliveryComplete.validate()) return;
                // view 객체 조회
